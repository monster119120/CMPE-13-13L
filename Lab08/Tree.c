 ...
 * Modified on February 28, 2016, 1:18 PM
 */
...
{
    // Allocate new node from memory
    Node *newItem = malloc(sizeof (Node));

    // Check if node was allocated from memory
    if (!newItem) {
        return STANDARD_ERROR;
    } else {
        // Store character in data field
        newItem->data = *data;

        if (level - 1) {
            // store left child node pointer
            newItem->leftChild = TreeCreate((level - 1), (data + 1));
            // store right child node ponter
            newItem->rightChild = TreeCreate((level - 1), (data + ((int) (pow(2, (level - 1))))));
        } else {
            // Nodes at lowest level will not have any left and right children
